...
-- ----------------------------
-- think_auth_rule，规则表，
-- id:主键，name：规则唯一标识, title：规则中文名称 type:类型（0存在规则就通过，1按规则表达式进行认证），condition：规则表达式
-- ----------------------------
...
class Authority {
    ...
    public function getAuth($name, $uid, $relation='or', $params=array()) {
        ...
        foreach ($authList as $val) {
            if (in_array($val['rule'], $name)){
                if(!empty($val['params'])){
                    $check = true;
                    foreach($val['params'] as $pk=>$pv){
                        if(!isset($params[$pk])||$pv!=$params[$pk]){
                            $check = false;
                            break;
                        }
                    }
    ...
    public function getGroups($uid) {
        ...
        if (!empty($groups[$uid]))
            return $groups[$uid];
        $groups[$uid] = Db::table($this->_config['auth_group_access'])->alias('a')->where("a.uid='$uid'")->join($this->_config['auth_group']." g","a.group_id = g.id")->select();
        return $groups[$uid];
    }
    ...
    protected function getAuthList($uid) {
        ...
        $authList = array();
        foreach ($rules as $r) {
            $params = empty($r['params']) ? array() : json_decode($r['params'],true);
            if ($r['type'] == 1) {
                //条件验证
                ...
                if ($condition) {
                    $authList[] = array('rule'=>$r['name'],'params'=>$params);
                }
            } else {
                //存在就通过
                $authList[] = array('rule'=>$r['name'],'params'=>$params);
