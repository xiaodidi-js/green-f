<template>
        ...
        <div id="scroller" class="ms-scroller" style="transform:translate3d(0px,0px,0px)">
            <div class="ms-item">
                ...
                <div id="content">
                    ...
                    <!-- 推荐产品 -->
                    <div class="tuijian-box" style="display:none;" @touchstart.stop @touchend.stop
                         v-show="data.commend">
                        <div id="scbox" class="tj-scroller">
                            <card-recommend :info="tjData" :card-width="10"></card-recommend>
                        </div>
                    </div>
                </div>
            </div>
    ...
    <div class="fpmasker" :class="{'show':formatPopShow}" @touchmove.stop.prevent @touchend.stop @touchstart.stop
         @click="hideFormatPop"></div>
    <div class="format-pop" :class="{'show':formatPopShow}" @touchmove.stop.prevent @touchend.stop @touchstart.stop>
        ...
        <div class="line" v-for="(pindex,fmt) in data.format">
            ...
            <div id="con" class="con" style="font-size:0;white-space:normal;">
                <guige :value="val.id" :text="val.name" v-for="(sindex,val) in fmt.value"
                ...
                if (this.data.activeid != 0) {
                    if (this.data.activepay > this.buyNums) {
                        ...
                        if (this.buyNums == this.data.activepay) {
                            this.buyNums = this.data.activepay;
                            alert("超出限购范围");
                        }
                    }
                    return false;
